...
export const organizationMemberships = pgTable(
  ...
  (table) => [
    unique("organization_memberships_unique_user_org").on(
...
export const organizationNombaAccounts = pgTable(
  "organization_nomba_accounts",
  {
    id: id(),
    organizationId: bigint("organization_id", { mode: "number" })
      .notNull()
  ...
  (table) => [
    index("organization_nomba_accounts_organization_id_idx").on(
      table.organizationId,
    ),
    index("organization_nomba_accounts_provider_account_id_idx").on(
      table.provider,
      table.providerAccountId,
    ),
    uniqueIndex("organization_nomba_accounts_one_default_per_org_idx")
...
export const organizationBankAccounts = pgTable(
  ...
  (table) => [
    unique("organization_bank_accounts_unique_per_org").on(
      table.organizationId,
      table.bankCode,
      table.accountNumber,
    ),
    index("organization_bank_accounts_organization_id_idx").on(
      table.organizationId,
    ),
    uniqueIndex("organization_bank_accounts_one_default_per_org_idx")
      .on(table.organizationId)
      .where(sql`${table.isDefault} = true`),
  ],
);
...
export const payers = pgTable(
  ...
  (table) => [
    unique("payers_unique_external_id_per_org").on(
      table.organizationId,
      table.externalId,
    ),
    index("payers_organization_id_idx").on(table.organizationId),
    index("payers_organization_name_idx").on(
      table.organizationId,
      table.fullName,
    ),
  ],
...
export const collections = pgTable(
  ...
  (table) => [
    ...
    index("collections_organization_status_idx").on(
      table.organizationId,
      table.status,
    ),
    index("collections_organization_due_date_idx").on(
      table.organizationId,
      table.dueDate,
    ),
  ],
...
export const collectionPayers = pgTable(
  "collection_payers",
  {
    id: id(),
    organizationId: bigint("organization_id", { mode: "number" })
  ...
  (table) => [
    unique("collection_payers_unique_payer_per_collection").on(
      table.collectionId,
      table.payerId,
    ),
...
export const invoices = pgTable(
  "invoices",
  {
    ...
    amountPaidKobo: bigint("amount_paid_kobo", { mode: "number" })
      .notNull()
      .default(0),
    currency: text("currency").notNull().default("NGN"),
    dueDate: date("due_date").notNull(),
  ...
  (table) => [
    unique("invoices_unique_payer_per_collection").on(
      table.collectionId,
      table.payerId,
    ),
...
export const invoicePaymentOptions = pgTable(
  "invoice_payment_options",
  {
    ...
    organizationNombaAccountId: bigint("organization_nomba_account_id", {
      mode: "number",
    }).references(() => organizationNombaAccounts.id),
    provider: text("provider").notNull().default("nomba"),
  ...
  (table) => [
    index("invoice_payment_options_organization_id_idx").on(
      table.organizationId,
    ),
    ...
    uniqueIndex("invoice_payment_options_unique_order_reference_idx")
      .on(table.provider, table.orderReference)
      .where(sql`${table.orderReference} is not null`),
    uniqueIndex("invoice_payment_options_one_active_type_per_invoice_idx")
      .on(table.invoiceId, table.optionType)
      .where(sql`${table.status} in ('pending', 'active')`),
  ],
);
...
export const webhookEvents = pgTable(
  ...
  (table) => [
    ...
    index("webhook_events_processing_status_idx").on(
...
export const payments = pgTable(
  "payments",
  {
    ...
    payerId: bigint("payer_id", { mode: "number" })
      .notNull()
      .references(() => payers.id),
    invoicePaymentOptionId: bigint("invoice_payment_option_id", {
      mode: "number",
    }).references(() => invoicePaymentOptions.id),
    organizationNombaAccountId: bigint("organization_nomba_account_id", {
      mode: "number",
    }).references(() => organizationNombaAccounts.id),
  ...
  (table) => [
    unique("payments_unique_provider_reference").on(
      table.provider,
...
export const reconciliationEvents = pgTable(
  ...
  (table) => [
    ...
    index("reconciliation_events_webhook_event_id_idx").on(
    ...
    index("reconciliation_events_status_idx").on(
      table.organizationId,
...
export const withdrawals = pgTable(
  "withdrawals",
  {
    ...
    organizationNombaAccountId: bigint("organization_nomba_account_id", {
      ...
      .notNull()
      .references(() => organizationNombaAccounts.id),
    destinationBankAccountId: bigint("destination_bank_account_id", {
      mode: "number",
    })
      .notNull()
      .references(() => organizationBankAccounts.id),
    amountKobo: bigint("amount_kobo", { mode: "number" }).notNull(),
    currency: text("currency").notNull().default("NGN"),
  ...
  (table) => [
    ...
    index("withdrawals_nomba_account_id_idx").on(
      table.organizationNombaAccountId,
    ),
    index("withdrawals_destination_bank_account_id_idx").on(
      table.destinationBankAccountId,
    ),
    index("withdrawals_requested_by_user_id_idx").on(table.requestedByUserId),
    index("withdrawals_organization_status_idx").on(
      table.organizationId,
      table.status,
    ...
    uniqueIndex("withdrawals_provider_reference_idx")
      .on(table.provider, table.providerReference)
      .where(sql`${table.providerReference} is not null`),
    index("withdrawals_ledger_entry_id_idx").on(table.ledgerEntryId),
  ],
);

export const organizationLedgerEntries = pgTable(
  "organization_ledger_entries",
  {
    ...
    withdrawalId: bigint("withdrawal_id", { mode: "number" }).references(
      () => withdrawals.id,
      { onDelete: "set null" },
    ),
  ...
  (table) => [
    unique("organization_ledger_entries_unique_reference").on(table.reference),
    index("organization_ledger_entries_organization_id_idx").on(
      table.organizationId,
    ),
    index("organization_ledger_entries_payment_id_idx").on(table.paymentId),
    index("organization_ledger_entries_withdrawal_id_idx").on(
      table.withdrawalId,
    ),
    index("organization_ledger_entries_org_created_at_idx").on(
      table.organizationId,
      table.createdAt.desc(),
    ),
  ],
